...
modelParams* ParametersManager::getModelParameters()
{
  return &modelParameters;
}
...
void ParametersManager::setTrajectoryParamsList(std::vector<trajectoryParams> &trajParamsList)
{
  trajectoryParametersList.clear();
  for (trajectoryParams &tp : trajParamsList)
  {
    trajectoryParametersList.push_back(std::move(tp));
  }
}
