...
#include "../include/FarmDataManager.h"
...
void FarmDataManager::readFarmFile( string farmSimulationFile)
{
	string line;
	ostringstream os;
	...
	input_tempfile="../AGRO_DATA/"+ farmSimulationFile;
	//sedCommand="sed  -e 's/[ ]*#.*//g' -e '/^$/d' ../AGRO_DATA/"+ farmSimulationFile+" > "+input_tempfile;
	...
	for(int tabIndice=0; tabIndice<nbParcels; tabIndice++)
	{
	...
	if(dataRoot.find("CYCLES_LIST")!=dataRoot.not_found())
	{
		...
		for (ptree::value_type &cy : cyclesTree.get_child(""))
		{
			...
			if(cycleRoot.find("SPECS_COURTES")!=cycleRoot.not_found())
			{
				...
				ptree::const_iterator it = tabTree.begin();
				for(int tabIndice=0; tabIndice<nbShortCrops; tabIndice++)
				{
...
void FarmDataManager::readAllCropsFile( string allCropsListFile)
{
	...
	input_tempfile="../AGRO_DATA/"+ allCropsListFile;
	//sedCommand="sed  -e 's/[ ]*#.*//g' -e '/^$/d' ../AGRO_DATA/"+ allCropsListFile+" > "+input_tempfile;
	//unsigned long long int zz=system(sedCommand.c_str()); zz++;
...
void FarmDataManager::readParamsFile( string numParamsFile){
	FILE * pFile;
	string line;
	...
	input_tempfile="../AGRO_DATA/"+ numParamsFile;
	//sedCommand="sed  -e 's/[ ]*#.*//g' -e '/^$/d' ../AGRO_DATA/"+ numParamsFile+" > "+input_tempfile;
	//unsigned long long int zz=system(sedCommand.c_str()); zz++;
...
FarmDataManager::FarmDataManager(string allCropsListFile, string farmDescriptionFile, string numParamsFile)
{
	// TODO Auto-generated constructor stub
	FILE * pFile;
	string line;
	...
	for(unsigned long long int i=0; i<nbPointsIBQS; i++)
	{
	...
	if(print)
	{
		for(int c=0;c<nbCycles;c++)
		{
			...
			for(unsigned long long int k=0; k<nbPointsIBQS; k++)
			{
	...
	{
		for(unsigned long long int t = 0; t<timeHorizon; t++)
		{
...
unsigned long long int FarmDataManager::getMaxCycleDuration()
{
	return maxCycleDuration;
}
double* FarmDataManager::getIgrid()
{
	return Igrid;
}
unsigned long long int FarmDataManager::getNbControls()
{
	return nbCycles;
}
unsigned long long int FarmDataManager::getNbIPoints()
{
	return nbPointsIBQS;
}
double FarmDataManager::getIStar()
{
	return IBQS_target;
}
unsigned long long int FarmDataManager::getTimeHorizon()
{
	return timeHorizon;
}
double FarmDataManager::getIbqsLevelForSavings()
{
	return IBQS_level_first_parcel;
}

double FarmDataManager::getCycleSeasonality( unsigned long long int un, unsigned long long int t)
{
	return seasonalities[un][t];
}

double FarmDataManager::getAdmissibleDeficit()
{
	return deficitMax;
}

unsigned long long int  FarmDataManager::getItransition ( unsigned long long int In, unsigned long long int t, unsigned long long int un)
{
...
unsigned long long int  FarmDataManager::getItransitionPrevious ( unsigned long long int In, unsigned long long int t, unsigned long long int un)
{
...
unsigned long long int FarmDataManager::getTimeTransition (  unsigned long long int t, unsigned long long int un)
{
	//cout<< " cycle duration is "<<cycleDurations[un]<<endl;
	return min(cycleDurations[un], timeHorizon - t);
}

unsigned long long int FarmDataManager::getCycleDuration (unsigned long long int un)
{
	//cout<< " cycle duration is "<<cycleDurations[un]<<endl;
	return cycleDurations[un];
}

double FarmDataManager::getCycleBalance(unsigned long long int In, unsigned long long int t, unsigned long long int un)
{
	if(t+cycleDurations[un]<=timeHorizon)
	{
...
double FarmDataManager::getCyclePerturbedBalance(unsigned long long int In, unsigned long long int t, unsigned long long int un, double v)
{
	if(t+cycleDurations[un]<=timeHorizon)
	{
...
double FarmDataManager::getCycleBalanceForGivenPeriod(unsigned long long int In, unsigned long long int mStart, unsigned long long int mEnd, unsigned long long int un)
{
	double res = 0.0;
...
double FarmDataManager::getCycleBalanceWithPricePerturbForGivenPeriod(unsigned long long int In,
...
double FarmDataManager::getMinCycleBalanceForGivenPeriod(unsigned long long int In, unsigned long long int gamma, unsigned long long int nbMonths)
{
	double res = PLUS_INF;
	for(int k=0;k<nbCycles;k++)
	{
...
double FarmDataManager::getCycleMaxDeficitForGivenPeriod(unsigned long long int In, unsigned long long int mStart, unsigned long long int mEnd, unsigned long long int un)
{
	double df=0.0;
...
double FarmDataManager::getCycleMaxDeficitForGivenPeriodMultiParcel(unsigned long long int *In, unsigned long long int *mStart, unsigned long long int nbMonths, unsigned long long int *un)
{
	double df=0.0;
	double dfMax=-PLUS_INF;
	//cout<<"DEFICIT MAX time period is "<<mStart<< " "<<mEnd<<endl;
...
double FarmDataManager::getCycleMaxDeficit(unsigned long long int In, unsigned long long int t, unsigned long long int un)
{
...
double FarmDataManager::getCycleMaxDeficitWithAnnexe(unsigned long long int In, unsigned long long int t, unsigned long long int un)
{
...
int FarmDataManager::getNbTrajs()
{
	return nbTrajectoires;
}
unsigned long long int * FarmDataManager::getInitPointsCoords()
{
	return initPointsCoords;
}
double * FarmDataManager::getInitValues()
{
	return initValues;
}

void FarmDataManager::saveFarmDataForMatlab()
{
	...
	{
		for(unsigned long long int l1=0; l1<nbPointsIBQS; l1++)
		{
			fprintf(fi,  "%15.8f\n" ,    Igrid[l1]);
		}
...
double FarmDataManager::getAnnexCycleBalance(unsigned long long start, unsigned long long int end)
{
	double result = 0.0;
...
int  FarmDataManager::getNbParcels()
{
	return nbParcels;
}

double * FarmDataManager::getParcelsSurfaces()
{
	return surfaces;
}

string FarmDataManager::getPrefix()
{
	return prefix;
}
