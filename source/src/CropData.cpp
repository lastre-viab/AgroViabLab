...
CropData::CropData(string fichierSource) {
	...
	input_tempfile="../AGRO_DATA/"+fichierSource;
	//sedCommand="sed  -e 's/[ ]*#.*//g' -e '/^$/d' ../AGRO_DATA/"+fichierSource+" > "+input_tempfile;
...
double CropData::calculBilanMois(unsigned long long int t, double I, double fixedCost)
{
	...
	if(t > static_cast<unsigned long long int>(duration))
	{
		return 0.0;
	}
	double s= sensi;
	...
	{
		bl=bl+(R*(pricePerTonn+productionGrantPerMonth)-croppingCost*(R/Rm)*(Rm>0))*(t == static_cast<unsigned long long int>(firstCroppingMonth));
		...
		if((t > static_cast<unsigned long long int>(firstCroppingMonth)) & (croppingPeriod>0))
		{
...
double CropData::calculDeltaBilanMois(unsigned long long int t, double I, double v)
{
	...
	if(Rm>0)
	{
		bl=bl+(R*(pricePerTonn*v))*(t == static_cast<unsigned long long int>(firstCroppingMonth));
		//cout<< " recolte cost "<<costs[4]*(R/Rm)*(Rm>0)<< " gain recolte "<<R*(p+Sp)<<endl;
...
double CropData::calculBilanMoisAssocie(unsigned long long int t, double I, double fixedCost, double wSurface, double lerPartiel)
{
	...
	{
		bl=bl+(R*(pricePerTonn+productionGrantPerMonth)-wSurface*croppingCost*(R/Rm))*(t == static_cast<unsigned long long int>(firstCroppingMonth));
		//cout<< " recolte cost "<<costs[4]*(R/Rm)*(Rm>0)<< " gain recolte "<<R*(p+Sp)<<endl;
		if(t==1)
		{
