...
void CycleData::computeCyclePartialData(string prefix, double * Igrid, unsigned long long int nbIPoints, unsigned long long int T)
{
	...
	for(unsigned long long int k=0; k<nbIPoints; k++)
		{
			cyclePartialIBQSTransitionMatrixPrevious[k]=new unsigned long long int [globalCycleDuration-1];
			for(unsigned long long int t=0; t<globalCycleDuration-1; t++)
			{
...
double CycleData::calculBilanCycleSingleCrop(double I,int t, int T)
{
	...
	if(static_cast<unsigned long long int>(t) + globalCycleDuration <= static_cast<unsigned long long int>(T))
	{
...
void CycleData::saveCyclePartialData(string prefix, unsigned long long int nbIPoints, double * Igrid)
{
	...
	{
		for(unsigned long long int t=0; t<globalCycleDuration; t++)
		{
			fprintf(fi,  "%llu " , t);
		}
		fprintf(fi,  "\n");
		for(unsigned long long int l1=0; l1<nbIPoints; l1++)
		{
	...
	else
	{
		...
		for(unsigned long long int l1=0; l1<nbIPoints; l1++)
		{
			...
			//unsigned long long int index =(unsigned long long int)min((int)nbIPoints - 1, max(0, (int)floor((IbqsComplet-Igrid[0])/(Igrid[1]-Igrid[0]))));
						fprintf(fi,  "%llu " , index);
			for(unsigned long long int t=0; t<globalCycleDuration-1; t++)
			{
				fprintf(fi,  "%llu " , cyclePartialIBQSTransitionMatrix[l1][t]);
			}
	...
	os<<"../OUTPUT/"<<prefix<<"-"<<cycleName<<"-PartialIBQSPrevious.dat";
		...
		else
		{
			...
			for(unsigned long long int l1=0; l1<nbIPoints; l1++)
			{
				...
				for(unsigned long long int t=0; t<globalCycleDuration-1; t++)
				{
					fprintf(fi,  "%llu " , cyclePartialIBQSTransitionMatrixPrevious[l1][t]);
				}
