...
#include "../models/agroEcoDiv_params_defs.h"
...
template <typename T>
T getUserSymbolOrDefaultTemplate(void *modelHandle, const char *symbolName,
                                 T defaultSymbol)
  {
...
template <typename T> T getUserSymbol(void *modelHandle, const char *symbolName)
  {
  T symbol = (T)dlsym(modelHandle, symbolName);
  if (symbol == nullptr)
    {
...
#define getUserSymbolOrDefault(handle, f)                                      \
  getUserSymbolOrDefaultTemplate(handle, #f, &f)

ParametersManager *initParams(void *modelHandle, const string &paramsFile,
                              gridParams &gp, algoViabiParams &avp,
                              controlParams &cp, systemParams &sp,
                              int nbOmpThreads)
  {

  ParametersManager *pm = new ParametersManager(
      &gp, &avp, &cp, &sp, nbOmpThreads, paramsFile, modelHandle);
  ...
  sp.CONSTR_XU_HYBRID =
      getUserSymbolOrDefault(modelHandle, constraintsXU_hybrid);
  ...
  sp.LOCAL_DYN_BOUNDS_HYBRID =
      getUserSymbolOrDefault(modelHandle, localDynBounds_hybrid);
  ...
  sp.CONTROL_ELIGIBILITY_FOR_TRAJ_fd =
      getUserSymbolOrDefault(modelHandle, controlEligibilityForTraj_fd);
  ...
  sp.DYN_CONSTR_FOR_TRAJ =
      getUserSymbolOrDefault(modelHandle, dynConstraintsForTraj);

  sp.MU_FUNC_FD = getUserSymbolOrDefault(modelHandle, constraintsXUY_fd);

  std::vector<trajectoryParams> &trajParamsList =
      pm->getTrajectoryParametersList();

  // Il ne peut pas y avoir de CONTROL_WEIGHT par défaut
  controlWeight_t controlWeight =
      getUserSymbol<controlWeight_t>(modelHandle, "controlWeight");
  neighborValidator_t validator =
      getUserSymbol<neighborValidator_t>(modelHandle, "isValidNeighbor");
  temporalControl_t temporalControl =
      getUserSymbol<temporalControl_t>(modelHandle, "temporalControl");
  userTyche_t userTyche = getUserSymbol<userTyche_t>(modelHandle, "tycheValue");
  cumulativeDistribution_t cumulativeDistribution =
      getUserSymbol<cumulativeDistribution_t>(modelHandle,
                                              "cumulativeDistribution");
  probabilityDensity_t probabilityDensity =
      getUserSymbol<probabilityDensity_t>(modelHandle, "probabilityDensity");
  ...
  for (trajectoryParams &tp : trajParamsList)
    ...
    case WEIGHTED_CONTROLS:
      ...
      if (tp.CONTROL_WEIGHT == nullptr)
        {
        spdlog::warn(
            "No control weight function defined even though WEIGHTED_CONTROLS "
            "was requested. Defaulting to arbitrary control order");
    ...
    case CUSTOM:
      if (tp.IS_VALID_NEIGHBOR == nullptr)
        {
        spdlog::warn("Custom bubble requested but no custom isValidNeighbor "
                     "function given. Defaulting to MOORE");
    ...
    case CUSTOM_PX:
      if (tp.IS_VALID_NEIGHBOR == nullptr)
        {
        spdlog::warn("Custom bubble requested but no custom isValidNeighbor "
                     "function given. Defaulting to MOORE_PX");
...
void initFarmData(ParametersManager *PM)
  {
  ...
  modelParams *mp = PM->getModelParameters();
  allCropsListFile = mp->getString("SPECLISTE");
  farmDescriptionFile = mp->getString("EXPLOITATION");
  numParamsFile = mp->getString("NUM_PARAMS");

  modelData =
      new FarmDataManager(allCropsListFile, farmDescriptionFile, numParamsFile);
  ...
  std::vector<trajectoryParams> tpList = PM->getTrajectoryParametersList();
  ...
  double IStar = modelData->getIStar();
  double *IRealValues = modelData->getIgrid();
  double iStep = IRealValues[1] - IRealValues[0];

  double ibqslevel = modelData->getIbqsLevelForSavings();

  gp->SLICE_VALUES_FD[0] = (unsigned long long int)floor(
      (gp->SLICE_VALUES[0] - IRealValues[0]) / iStep);
  ...
  std::vector<trajectoryParams> trajectoryParamsList =
      std::vector<trajectoryParams>();
  trajectoryParamsList.clear();

  unsigned long long int *tempinitPoints = modelData->getInitPointsCoords();
  double *tempInitiValues = modelData->getInitValues();
  for (int j = 0; j < nbTrajs; j++)
    {
    trajectoryParams trajParams;
    trajParams.maxTime = (double)(modelData->getTimeHorizon());

    trajParams.NB_STRATEGIES = 0;
    trajParams.TRAJECTORY_TYPE = VD;

    trajParams.INIT_POINT = new double[gp->DIM];

    unsigned long long int *initPoint_fd = new unsigned long long int[gp->DIM];
    for (int k = 0; k < dim; k++)
      {
      initPoint_fd[k] = tempinitPoints[k + j * dim];
      }
    trajParams.INIT_POINT_FD = initPoint_fd;

    trajParams.INIT_VALUE = tempInitiValues[j];
    trajParams.INIT_VALUE_FD = tempInitiValues[j];

    trajParams.INIT_CONTROL = new double[cp->DIMC];
    trajParams.REAL_TIME_STEPS_PER_DISCRETE_STEP = 1;
    trajParams.MAX_ANGLE_RADIANS = pi / 2;

    trajParams.SAVE_PICKING_STRATEGY = false;
    trajParams.ARE_STRATEGIES_GUARANTEED = false;
    trajectoryParamsList.push_back(std::move(trajParams));
    }

  PM->setTrajectoryParamsList(trajectoryParamsList);
  }
